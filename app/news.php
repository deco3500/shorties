<!-- news -->
<script type="text/javascript" src="js/news.js"></script>
<div class="load_continer">
	<div class="media">
		
	</div>
	<div class="MSG_Box">
		<div class="poll form-group hvr-hang">
				
			<img src="img/lol.png" alt="LOL" height="40" width="40"/>
			<span class="label label-default" >LOL</span>
			<img src="img/yas.png" alt="YAS" height="40" width="40"/>
			<span class="label label-default" >YAS</span>
			<img src="img/wtf.png" alt="WTF" height="40" width="40"/>
			<span class="label label-default" >WTF</span>
			<img src="img/omg.png" alt="OMG" height="40" width="40"/>
			<span class="label label-default" >OMG</span>				
			
		</div>
		<div class="msg input-group">
			<textarea class="form-control" type="text" maxlength="225"/>
			<div class="input-group-btn">
				<button type="button" class="btn btn-default"><img src="img/send.png" width="40" height="40" alt="Send"></button>
  			</div>
		</div>
			
	</div>
</div>